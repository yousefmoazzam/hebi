...
if [ -z "{$ACTIVATE_CAS_AUTH+x}" ]; then
    echo "ACTIVATE_CAS_AUTH is not set, assume 'True' for production"
    sed -i 's/DUMMY_ACTIVATE_CAS_AUTH/'"'True'"'/g' /var/cache/nginx/config/www/webpack.config.js
else
...
# set CAS_SERVER in webpack config (for authentication)
if [ -z "{$CAS_SERVER+x}" ]; then
    echo "CAS_SERVER is not set, so set to an empty string: note that if CAS auth is on, then an empty string value here will error during runtime when trying auth, so need to catch this somehow in the future!"
...
# set SERVICE in webpack config (for authentication)
if [ -z "{$SERVICE+x}" ]; then
    echo "SERVICE is not set, so set to an empty string: note that if CAS auth is on, then an empty string value here will error during runtime when trying auth, so need to catch this somehow in the future!"
    sed -i 's/DUMMY_CAS_SERVER/''/g' /var/cache/nginx/config/www/webpack.config.js
else
...
if [ -z "{$ACTIVATE_WEBSOCKET+x}" ]; then
    echo "ACTIVATE_WEBSOCKET env var is not set, assume True for production"
    sed -i 's/DUMMY_ACTIVATE_WEBSOCKET/'"'True'"'/g' /var/cache/nginx/config/www/webpack.config.js
else
