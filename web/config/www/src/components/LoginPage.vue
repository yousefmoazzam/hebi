<template>
  <div id="root">
    <h1 class="flex justify-center text-4xl">Log into Hebi</h1>
    <div class="flex justify-center">
      <button v-if="!inProcessOfAuthenticating && !isAuthenticated && !isUnauthorisedUser"
        class="rounded bg-blue-400 text-white m-1 p-1"
    ...
    <div v-if="isAuthenticated">
      ...
      <p class="flex justify-center">
        You will shortly be redirected to the process list editor
      </p>
    </div>
    <div v-if="isUnauthorisedUser" class="flex justify-center">
      <p>
        {{ 'Sorry, ' + this.unauthorisedUser + ' isn\'t authorised to access this Hebi session.' }}
      </p>
    </div>
  </div>
</template>
    ...
    this.checkForCookie()
      .then(resp => {
        if (!resp) {
          ...
          console.log('no CAS cookie in browser, checking for ticket in url')
          ...
          //   re-authentication
          // - having no ticket when already authenticated isn't a problem
          //   either
          // so can just redirect to Hebi homepage
  ...
  methods: {
    ...
    checkFailedTicketValidation: function (authResp) {
      if ('code' in authResp && authResp.code === 'INVALID_TICKET') {
        ...
        // a user that doesn't own the session has been denied access to the
        // session
