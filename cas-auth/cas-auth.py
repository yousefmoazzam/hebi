...
def check_for_cookie():
    '''
    Check if the HTTP request that came from Hebi in the web browser has a
    cookie that denotes if the user requesting access is the owner of the Hebi
    session

    If there is a cookie present, it needs to be decoded to check if the user
    is the owner of the Hebi session
    '''
    ...
    if cookie is None:
        ...
        if 'username' not in decoded_token:
            # something wrong with the token, so deny access
            payload['is_requestor_session_owner'] = False
            return jsonify(payload)

        payload['username'] = decoded_token['username']
        if decoded_token['username'] == os.environ['FEDID']:
            payload['is_requestor_session_owner'] = True
        else:
            payload['is_requestor_session_owner'] = False
...
def validate_ticket():
    ...
    if 'authenticationSuccess' in auth_resp['serviceResponse']:
        ...
        # check if the user who has authenticated with CAS matches the owner of
        # the Hebi session
        if username == os.environ['FEDID']:
            output_dict['validated'] = True
            output_dict['user'] = username
            output_dict['desc'] = 'successful authentication'

            # create token from the ticket validation that has the user's FedID
            # stored in it
            payload = {
                'username': username
            }
            token = jwt.encode(payload,  os.environ['JWT_KEY'],
            ...
            # the user who has attempted to log into the Hebi session is not
            # the owner of the session, so reject the request
            output_dict['validated'] = False
            output_dict['desc'] = 'unauthorised user access'
            output_dict['unauthorised_user'] = username
            resp = jsonify(output_dict)
    elif 'authenticationFailure' in auth_resp['serviceResponse']:
        output_dict['validated'] = False
        output_dict['code'] = \
            auth_resp['serviceResponse']['authenticationFailure']['code']
        output_dict['desc'] = \
            auth_resp['serviceResponse']['authenticationFailure']['description']
        resp = jsonify(output_dict)
    else:
...
def logout():
    '''
    Delete cookie in the user's web browser to log out of their Hebi session
    '''
    # delete cookie generated from CAS validation
...
def main(argv):
    app.run(host='0.0.0.0', port=8090, debug=True, use_reloader=True,
        threaded=True)
